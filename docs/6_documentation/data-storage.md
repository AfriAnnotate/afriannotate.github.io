...
Store the data in open, standard formats that others can read without special tools, and pair it with machine-readable metadata describing its contents, following the documentation standards from this chapter. Keep the raw collection separate from processed versions, and never overwrite the original. Version every release with a number and a changelog, so that results stay reproducible and users can cite the exact data they used, which is the same versioning discipline the [Data Quality](../4_data-quality/index.md) and [Dataset Lifecycle](../9_dataset-lifecycle/maintenance.md) chapters require.
